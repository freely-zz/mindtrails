...
public class Andy_New: IPlayerA
{
    ...
    public List<Actions> MakeDecision(List<Vector3> blocks, int thisScore, float timeRemaining)
    {
        ...
        if (paths[keyList[0]].Count > 0)
        {
            ...
            else
            {
                ...
                redsToPlace = predictNext(totReds, 100);
        ...
        //Methods.HighlightCells(paths[keyList[0]]); Highlights removed for Wally version
    ...
    public void iniSuspicion()
    {
        foreach (var key in keyList)
        {
            suspicion.Add(key, 1f / zones.Count);
        }
    }
    ...
    public List<Vector3> predictNext(int redsNum, int predictCount)
    {
        ...
        Dictionary<List<Vector3>, float> susPcopy = new Dictionary<List<Vector3>, float>();
        ...
        for (i = 0; i < predictCount; i++)
        {
            ...
            {
                Score = tempScore;
                susPcopy = new Dictionary<List<Vector3>, float>(susP);
        ...
        foreach (var key in keyList)
        {
            foreach (var w in weights)
            {
                if (key != w.Key)
                {
                    ...
                    UnityEngine.Debug.Log(susCopy[key]);
                }
            }
        }
    ...
    public List<Vector3> pathGen(Vector3 a1, Vector3 a2)
    {
        ...
        if (amb == 1)
        {
            if (allBlocks.Contains(mid) || allTokens.Contains(mid))
            {
                mid = randNearBlock(mid, gridSize / 10);
            }
            path.AddRange(PathFinder.StripPath(PathFinder.GetPath(a1, mid)));
            path.AddRange(PathFinder.StripPath(PathFinder.GetPath(mid, a2)));
        } else
        {
        ...
        foreach (var b in PathFinder.StripPath(PathFinder.GetPath(a1, mid)))
        {
    ...
    public float susCalc(float conf, float susWeight)
    {
        ...
        float learningRate = 1f / 3f;
        return susWeight * (learningRate * (conf + (float)Math.Exp(turn / 100)));
    }
    ...
    public float BoardScore(float suspicion, List<Vector3> distribution)
    {
        ...
        if (alpha == 0)
        {
            return 1.0f / suspicion;
        }
        else if (beta == 0)
        {
            return ((float)newRed / (float)pathLength);
        }
        return alpha * ((float)newRed / (float)pathLength) * beta * 1.0f / suspicion;
    }

    /*public void printStats(Dictionary<List<Vector3>, float> susP, float score)
    {
        List<string> output = new List<string>();
        foreach (var b in susP)
        {
            output.Add($"anchor {b.Key[0]} and {b.Key[1]}'s suspicion is {b.Value}");
        }
        output.Add($"board score is {score}");
    ...
    public void ini_mid()
    {
        ...
        mid = new Vector3((int)tx / allAnchors.Count, (int)ty / allAnchors.Count, 0);
    }
}
