...
public class rj_model_1: IPlayerA
{
    ...
    public List<Actions> MakeDecision(List<Vector3> blocks, int thisScore, float timeRemaining)
    {
        ...
        if (paths[keyList[0]].Count > 0)
        {
            ...
            else
            {
                ...
                tokensToPlace = predictNext(totReds, tot_NONreds, 10);
    ...
    public List<Vector3> predictNext(int redsNum, int NON_redsNum, int predictCount)
    {
        ...
        {   int T_pathindex = 0;
            ...
            //for each red available this turn, add it to  along TRUE path.
            for (j = 0; j < redsNum; j++)
            {
                if (T_pathindex < (paths[keyList[0]].Count) && Methods.IsEmptyGrid(paths[keyList[0]][T_pathindex]))
                {
                    temp = paths[keyList[0]][T_pathindex];
                    T_pathindex++;  
            ...
            {
                temp = Methods.GetRandomEmptyGrid(1)[0];
                //avoid duplications.
    ...
    public float Reward_fun(Dictionary<List<Vector3>, float> belief_P, Dictionary<List<Vector3>, float> suspition_P, float MIN_susp, List<Vector3> distribution)
    {
        ...
        return ((1 - (belief_P[trueAnchors] - Belief[trueAnchors])) + ((float)newRed / (float)pathLength) + (Confidence * (MIN_susp - suspition_P[trueAnchors])) );
    }
