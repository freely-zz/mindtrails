...
public class rj_model_2: IPlayerA
{
    ...
    // to show the state of the environment - how it look to opponent - ratio of reds and non-reds in a zone
    ...
    // private Dictionary<List<Vector3>, float> suspicion11 = new Dictionary<List<Vector3>, float>();
    ...
    public List<Actions> MakeDecision(List<Vector3> blocks, int thisScore, float timeRemaining)
    {
        ...
        else if (turn > 0)
        {
            ...
            // show_belief();// outputs belief value            
            // show_Trust(); // outputs - state of environment - values
            if(shuttles == 2)
            {
        ...
        if (paths[keyList[0]].Count > 0)
        {
            ...
            else
            {
                ...
                tokensToPlace = predictNext(totReds, tot_NONreds, 50);
    ...
    public List<Vector3> predictNext(int redsNum, int NON_redsNum, int predictCount)
    {
        ...
        // int non_redsNum = 8 - redsNum; ------ need to get RED_distribution and NON_RED_distribution
        ...
        for (i = 0; i < predictCount; i++)
        {
            ...
            for (j = 0; j < redsNum; j++)
            {
                temp = paths[keyList[0]][Random.Range(0, paths[keyList[0]].Count)];
                //avoid duplications.
        ...
        //to show state of environment at the first round
        // if (turn == 0)
        // {
        //     show_Trust();
        // }
    ...
    public float Reward_fun(Dictionary<List<Vector3>, float> belief_P, Dictionary<List<Vector3>, float> suspition_P, float MIN_susp, List<Vector3> distribution)
    {
        ...
        return ((1 - (belief_P[trueAnchors] - Belief[trueAnchors])) + (turn+((float)newRed / (float)pathLength)) + (MIN_susp - suspition_P[trueAnchors]) );
    }
    ...
    // returns the minimum suspition value 
