...
public class A_scatter : IPlayerA
{
    ...
    private List<Vector3> notEmptyGrids = new List<Vector3>();
    ...
    public List<Vector3> RegisterTrueAnchors()
    {
        ...
        //only being kept so functions arguments remain the same as A_testing.
    ...
    public List<Actions> MakeDecision(List<Vector3> blocks, int thisScore, float timeRemaining)
    {
        ...
        //SCORE FUNCTION
        Vector3 minBlock = CalcScore2(blocks, 0);
    ...
    //based on how many blocks were in the zone. each one in zone = -1 reward
    //not used
    public Vector3 CalcScore3(List<Vector3> blocks, float score)
    {
        totScore = blocks.Intersect(trueGrid).ToList().Count;
        return blocks.Intersect(trueGrid).ToList()[Random.Range(0, blocks.Intersect(trueGrid).ToList().Count)];
    }

    //based on how close to the path the blocks are, closer = smaller score.
    public Vector3 CalcScore2(List<Vector3> blocks, float score)
    {
        float dist;
        Vector3 minBlock = Vector3.zero;
        float minScore = Mathf.Infinity;

        //true path, but only the non-empty grids are taken into consideration
        List<Vector3> pathSoFar = plannedPath.Intersect(notEmptyGrids).ToList();
        //true path but doesn't take into account for empty grids
        //List<Vector3> pathSoFar = plannedPath;

        foreach (var b in blocks)
        {
            dist = Mathf.Infinity;
            //find distane to cloest token in path
            foreach (var cell in pathSoFar)
            {
                if (dist < Vector3.Distance(cell, b))
                {
                    dist = Vector3.Distance(cell, b);
                }
            }
        ...
        //return closest block to true path
        return minBlock;
    }
    ...
    //get all zones between all anchors, but with true zone removed from all of them.
    public void getAllZones()
    {
        ...
        for (int i = 0; i < anchors.Count; i++)
        {
            ...
            {
                Regions.Add(selectZone(anchors[i], anchors[j]).Except(trueGrid).ToList());
                indexer++;
            }
        }
        Regions.Remove(trueGrid);
    }

    //try to get a probability list and generate a random number based on it
    public int GetRandWeightInd()
    {
        List<int> weightList = new List<int>();
        foreach (List<Vector3> zone in Regions)
        {
            //more empty grids = higher chance of being weighted.
            weightList.Add(zone.Count - (zone.Intersect(notEmptyGrids)).ToList().Count);
        }
        return weightedRandomIndex(weightList);
    }

    //returns a random index for a zone based on randomness.
    public int weightedRandomIndex(List<int> weightList)
    {
        int r = Random.Range(0, weightList.Sum());
        int sum = 0;
    ...
    //return a random cell in a zone.
    public Vector3 randCellInZone(List<Vector3> zone)
    {
        List<Vector3> availableSpots = zone.Except(notEmptyGrids).ToList();
        ...
        //random indice in list.
        int r = Random.Range(0, availableSpots.Count);

        return availableSpots[r];
    }
    ...
    {
        // if the total distance of block to path is less than 6
        if (totScore < 6)
        {
            safeDepLogic(gridnum1, actions1, plannedPath, fakePath, i, minBlock);
        }
        else
        {
            normalDepLogic2(gridnum1, actions1, plannedPath, fakePath, i, minBlock);
        }
    }

    //logic for trying to avoid the real zone
    public void safeDepLogic(int gridnum1, Actions actions1, List<Vector3> plannedPath, List<Vector3> fakePath, int i, Vector3 minBlock)
    {
        float e = Random.Range(0.0f, 1.0f);
        //very high chance of putting the token in anywhere but the true zone.
        if (e < 0.9f)
        {
            Vector3 rwGrid = randCellInZone(Regions[GetRandWeightInd()]);
            actions1.DepositAt(rwGrid);
            notEmptyGrids.Add(rwGrid);
        }
        else
        {
            //if token isn't red, put randomly bin true zone.
            if (!Methods.IsRed(gridnum1, i))
            {
                Vector3 rwGrid = randCellInZone(trueGrid);
                actions1.DepositAt(rwGrid);
                notEmptyGrids.Add(rwGrid);
            }
            //red, but place as normal, extremely low chance.
            else if (Tpathindex < plannedPath.Count && Methods.IsEmptyGrid(plannedPath[Tpathindex]))
            {
                actions1.DepositAt(plannedPath[Tpathindex]);
                notEmptyGrids.Add(plannedPath[Tpathindex]);
                Tpathindex++;
            }
            //safekeeping
            else
            {
                Vector3 rGrid = Methods.GetRandomEmptyGrid(1)[0];
                actions1.DepositAt(rGrid);
                notEmptyGrids.Add(rGrid);
            }
        }
    }

    //logic for depositing blocks.
    public void normalDepLogic2(int gridnum1, Actions actions1, List<Vector3> plannedPath, List<Vector3> fakePath, int i, Vector3 minBlock)
    {
        float e = Random.Range(0.0f, 1.0f);
        //token is red.
        if (Methods.IsRed(gridnum1, i))
        {
            if (e < 0.6 && Tpathindex < plannedPath.Count && Methods.IsEmptyGrid(plannedPath[Tpathindex]))
            {
                actions1.DepositAt(plannedPath[Tpathindex]);
                notEmptyGrids.Add(plannedPath[Tpathindex]);
                Tpathindex++;
            }
            //random number, if less than 20 tokens placed, put red randomly
            else if (notEmptyGrids.Count > 20)
            {
                Vector3 rGrid = Methods.GetRandomEmptyGrid(1)[0];
                actions1.DepositAt(rGrid);
                notEmptyGrids.Add(rGrid);
        ...
        else
        {
            ...
            if (minBlock != Vector3.zero)
            {
                if (e < 0.8)
                {
                    //put token near that block with variance.
